...
Run `npx nx serve ui-webmethods.io` to start the development server. Happy coding!

## Build for production

Run `npx nx build ui-webmethods.io` to build the application. The build artifacts are stored in the output directory (e.g. `dist/` or `build/`), ready to be deployed.
...
1. Identify all the official Nx plugins that are used in your repo. This includes nx and any packages in the @nx/ or @nrwl/ organization scope, except for plugins that are still in nx-labs. Also, nx-cloud does not need to match the other package versions.
